...
export async function POST(req: Request) {
  try {
    const {
      ...
      folderId,
      folderName,
    }: { messages: CoreMessage[]; folderId: string; folderName: string } =
      await req.json()
    ...
    const systemMessage: CoreSystemMessage = {
      role: "system",
      content:
        `You are a intelligent note-taking app . Notes are stored in folders . The user has chosen the folder : ${folderName} . There are a total of ${totalNotesCount} in this folder. Structure your answers based on the relevant notes . Do not give generalised information or assumptions ` +

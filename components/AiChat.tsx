...
const AiChat = ({folderId, folderName } : {folderId : string, folderName : string}) => {
    ...
    <>
        ...
        >
            <CardContent >
                ...
                <div className='flex items-center justify-between '>
                    <div className='flex gap-3 items-center'>
                        <div className='bg-green-300 p-2 rounded-full'><Bot size={20}/></div>
                        <div >
                            <CardTitle className="text-lg font-semibold">Notefinity Ai</CardTitle>
                ...
                <main className='flex flex-col gap-2 overflow-y-auto py-2 text-sm h-[400px]' ref={scrollRef}>
                    {messages.map((message) => {
                        ...
                        {isLoading && <div className='w-full text-primary-foreground flex justify-start items-center gap-2 font-serif' key={"loading"}>
                            <Bot className='text-foreground' />
            ...
            <CardFooter>
                <form className='flex w-full gap-1' onSubmit={handleSubmit}>
                    <Input value={input} className='flex-1' onChange={handleInputChange} placeholder="What's on your mind ?"/>
      ...
      {!isOpen && (
        <div className="fixed bottom-4 right-4 z-50">
          <Button onClick={() => setIsOpen(true)} className="flex items-center gap-2 shadow-lg" size={"icon"}>

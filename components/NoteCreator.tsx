      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormControl>
                <TextareaAutoResize
                  className="outline-none text-2xl max-h-fit"
                  {...field}
                  ref={inputRef}
                  placeholder="Title"
                />
        ...
        <div className="flex items-center gap-2 justify-end">
              ...
              <FormItem>
                <FormControl>
                  ...
                  >
                    <Star
                      className={cn(
                        field.value && "stroke-amber-400 fill-amber-400",
                      )}
                    />
                  </Button>
                </FormControl>
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? <ClipLoader size={15} color="white" /> : "Create Note"}
          </Button>
        </div>
      </form>
    </Form>
